def fetch_and_save_donki_gst_data(start_date, end_date):
    try:
        ...
        data = response.json()
        donki_gst_csv_file_name = f"APIs/{datetime.now().strftime('%Y_%m_%d')}_donki_gst.csv"
        ...
        publish_csv_to_mqtt(donki_gst_csv_file_name, "api/data")
...
def fetch_and_save_temperature_api(start_date, end_date):
            try:
                ...
                # CSV file path
                noaa_temp_csv_file_name = f"APIs/{datetime.now().strftime('%Y_%m_%d')}_noaa_temp.csv"
                ...
                publish_csv_to_mqtt(noaa_temp_csv_file_name, "api/data")
...
def parse_UAH_SWIRLL(line):
                ...
                hour_minute = parts[3]
                seconds = parts[4]
                date_time = datetime.strptime(f"{year} {julian_day} {hour_minute}:{seconds}", "%Y %j %H%M:%S").strftime(
                    "%Y-%m-%d %H:%M:%S")
